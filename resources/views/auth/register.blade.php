...
@section('content')
    <div class="min-h-screen flex flex-col sm:justify-center items-center px-4 pt-6 sm:pt-0 bg-white sm:max-w-md m-auto">
       <p class="font-bold"><img class="h-24" src="{{ URL::to('/logo.png') }}"></p>
        <!-- <div class="font-sans text-3xl font-bold text-center pt-5 m-4">{{ __('Sign Up') }}</div> -->
        <div class="w-full mt-6 p-2 sm:p-6 py-6 bg-white border border-gray-100 overflow-hidden sm:rounded-lg">
            <form method="POST" action="{{ route('register') }}">
                ...
                <div class="row mt-3" x-data="{ show: true }">
                    <span class="block font-semibold text-base text-black">{{ __('Password') }}</span>
                    <div class="relative">
                        <input name="password" autocomplete="new-password" placeholder="" :type="show ? 'password' : 'text'"
                            class="text-md block px-3  placeholder-gray-600 shadow-md bg-gray-100 w-full mt-1 p-2 sm:p-2 py-2 border border-gray-100 overflow-hidden rounded-md focus:ring-1 focus:border focus:outline-none @error('password') is-invalid @enderror">
                ...
                <div class="row mt-3" x-data="{ show: true }">
                    <span class="block font-semibold text-base text-black">{{ __('Confirm Password') }} </span>
                    <div class="relative">
                        <input name="password_confirmation" autocomplete="new-password" placeholder=""
                ...
                <div class="text-center mt-3">
                    <p class="text-sm">{{ __('Already have an account. Click here to ') }}<a href="{{ route('login') }}"
                            class="text-blue-600">{{ _('Login') }}</a></p>
                </div>
            </form>
        </div>
    </div>
